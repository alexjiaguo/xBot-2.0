...
const server = new Server(
  { name: "gdrive-write", version: "0.1.0" },
...
server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
  ...
  if (mime.startsWith("text/") || mime === "application/json") {
    return { contents: [{ uri: request.params.uri, mimeType: mime, text: Buffer.from(res.data as ArrayBuffer).toString("utf-8") }] };
  }
  return { contents: [{ uri: request.params.uri, mimeType: mime, blob: Buffer.from(res.data as ArrayBuffer).toString("base64") }] };
});
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  if (name === "create_file") {
    ...
    const parents = Array.isArray(args?.parents) ? (args?.parents as string[]) : undefined;
    ...
    const media = args?.content ? { body: args?.content as string } : undefined;
